<aside
    ...
    class="fixed md:relative inset-y-0 left-0 z-40 w-64 bg-[#00479b] text-white flex flex-col p-6 shrink-0 transition-transform duration-300 ease-in-out md:translate-x-0">

    <div class="md:hidden flex justify-end">
        <button @click="sidebarOpen = false" class="text-white hover:text-gray-300 transition-colors">
            <i class="fas fa-times text-2xl"></i>
        </button>
    </div>

    <div class="mb-10 mt-2 text-center block">
    ...
    <nav class="flex-1 space-y-4 overflow-y-auto px-2">
        ...
        <div x-data="{ open: {{ request()->routeIs('berita.*') ? 'true' : 'false' }} }">
            <button @click="open = !open"
                ...
                {{ request()->routeIs('berita.*') ? 'bg-white/20 border-white text-white' : 'bg-white border-transparent text-[#00479b]' }}">
                Berita <i class="fas fa-chevron-down text-[10px] transition-transform" :class="open ? 'rotate-180' : ''"></i>
            </button>

            <div x-show="open" x-transition class="mt-2 space-y-1 bg-[#003d82] rounded-2xl p-1">
                <a href="{{ route('berita.klinik') }}"
                    class="block py-2 font-black text-center uppercase text-[10px] rounded-xl transition
                    {{ request()->routeIs('berita.klinik') ? 'bg-white/30 text-white' : 'text-white hover:bg-blue-500' }}">
                    Klinik
                </a>
                <a href="{{ route('berita.kartini') }}"
                    class="block py-2 font-black text-center uppercase text-[10px] rounded-xl transition
                    {{ request()->routeIs('berita.kartini') ? 'bg-white/30 text-white' : 'text-white hover:bg-blue-500' }}">
                    Kartini
                </a>
            </div>
        </div>
